    ...
    def do_sweep(key_index):
        ...
        if key_index < 0:
            for rep in range(repetitions):
                ...
                if (lr == 0.0001 and batch_size >= 32) or (lr != 0.0001 and batch_size != 256):
...
def main():
    ...
    tasks = sorted(tasks, key=lambda t: (
        t.depth + numpy.abs(numpy.log(t.run_config['batch_size']) - numpy.log(256)) + numpy.abs(numpy.log(t.optimizer['config']['learning_rate']) - numpy.log(0.0001)), t.dataset, numpy.random.randint(10000), t.seed))
