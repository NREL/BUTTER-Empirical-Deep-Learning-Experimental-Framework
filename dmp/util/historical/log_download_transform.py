...
connection_string = "postgresql://{user}:{password}@{host}:5432/{database}".format(
    **_credentials
)
...
drop_list = [
    ...
    "config.datasets",
    "config.jq_module",
    "config.run_config.shuffle",
    "config.run_config.verbose",
    "config.topologies",
    "config.epoch_scale.b",
    "config.epoch_scale.m",
    "config.label_noises",
    "config.early_stopping",
    "config.learning_rates",
    "config.residual_modes",
...
rename_map = {
    ...
    for s in [
        ...
        "history.squared_hinge",
        "history.cosine_similarity",
        "history.test_squared_hinge",
        "history.mean_squared_error",
        "history.mean_absolute_error",
        "history.test_cosine_similarity",
        "history.test_mean_squared_error",
        "history.root_mean_squared_error",
        "history.test_mean_absolute_error",
...
rename_map.update(
    {
        ...
        for s in [
            ...
            "config.activation",
            "config.num_hidden",
            "config.run_config.epochs",
            "config.run_config.shuffle",
            "config.run_config.verbose",
            "config.run_config.batch_size",
            "config.run_config.validation_split",
            "config.topologies",
            "config.epoch_scale.b",
            "config.epoch_scale.m",
            "config.label_noise",
...
def postprocess_dataframe(data_log, engine):
    ...
    col_set = set(datasets.columns)
    datasets.rename(
        columns={k: v for k, v in rename_map.items() if k in col_set}, inplace=True
    )
    ...
    datasets["runtime"] = datasets["runtime"].apply(
        lambda s: 0 if pd.isna(s) else str(int(s))
    )
    ...
    def canonicalize_string(s):
        ...
        engine.execute(
            "INSERT INTO strings(value) VALUES(%s) ON CONFLICT(value) DO NOTHING", (s,)
        )
        str_id = engine.execute(
            "SELECT id from strings WHERE value = %s", (s,)
        ).scalar()
        string_map[s] = str_id
        return str_id
    ...
    datasets.drop(
        columns=[c for c in datasets.columns if c not in dst_cols], inplace=True
    )
...
def func():
    ...
    string_map_df = pd.read_sql(
        ...
        engine.execution_options(stream_results=True, postgresql_with_hold=True),
        coerce_float=False,
        params=(),
    )
    ...
    print(
        f"Reading {count} records with read_size {read_size} and {num_readers} readers..."
    )
        ...
        for i in range(num_chunks):
            ...
            chunk = pd.read_sql(
                ...
                engine.execution_options(
                    stream_results=True, postgresql_with_hold=True
                ),
                coerce_float=False,
                params=(),
            )
