...
def main():
    ...
    return_values = [
        ("num", "uint8"),
        ("test_loss_num_finite", "uint8"),
        ("test_loss_avg", "float32"),
        ("test_loss_stddev", "float32"),
        ("test_loss_min", "float32"),
        ("test_loss_max", "float32"),
        ("train_loss_num_finite", "uint8"),
        ("train_loss_avg", "float32"),
        ("train_loss_stddev", "float32"),
        ("train_loss_min", "float32"),
        ("train_loss_max", "float32"),
        ("train_loss_median", "float32"),
        ("test_accuracy_avg", "float32"),
        ("test_accuracy_stddev", "float32"),
        ("test_accuracy_median", "float32"),
        ("test_loss_median", "float32"),
        ("train_accuracy_avg", "float32"),
        ("train_accuracy_stddev", "float32"),
        ("train_accuracy_median", "float32"),
        ("test_mean_squared_error_avg", "float32"),
        ("test_mean_squared_error_stddev", "float32"),
        ("test_mean_squared_error_median", "float32"),
        ("train_mean_squared_error_avg", "float32"),
        ("train_mean_squared_error_stddev", "float32"),
        ("train_mean_squared_error_median", "float32"),
        ("test_kullback_leibler_divergence_avg", "float32"),
        ("test_kullback_leibler_divergence_stddev", "float32"),
        ("test_kullback_leibler_divergence_median", "float32"),
        ("train_kullback_leibler_divergence_avg", "float32"),
        ("train_kullback_leibler_divergence_stddev", "float32"),
        ("train_kullback_leibler_divergence_median", "float32"),
    ]
    ...
    all_kinds = list((p[0] for p in fixed_parameters)) + variable_parameter_kinds
    ...
    for dim, kind in enumerate(all_kinds):
        ...
        for i, (value, id) in enumerate(
            sorted(parameter_map.get_all_parameters_for_kind(kind))
        ):
    ...
    with h5py.File(file_name, "w") as f:
        ...
        make_parameter_dataset(
            "kind", h5py.special_dtype(vlen=str), 0, "What does this parameter define?"
        )
        make_parameter_dataset(
            "value",
            h5py.special_dtype(vlen=str),
            1,
            "What value of this parameter's kind does this parameter indicate?",
        )
        make_parameter_dataset(
            "dimension",
            "uint8",
            2,
            "What dimension of data datasets indexes this parameter kind?",
        )
        make_parameter_dataset(
            "index",
            "uint8",
            3,
            "What index of of the dimension indicates this parameter value?",
        )
