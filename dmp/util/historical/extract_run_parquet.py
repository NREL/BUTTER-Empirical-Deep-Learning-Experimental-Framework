...
def main():
    ...
    data_columns = [
        ...
        pyarrow.field(
            "train_root_mean_squared_error", pyarrow.list_(pyarrow.float32())
        ),
        pyarrow.field(
            "test_mean_squared_logarithmic_error", pyarrow.list_(pyarrow.float32())
        ),
        pyarrow.field(
            "train_mean_squared_logarithmic_error", pyarrow.list_(pyarrow.float32())
        ),
        ...
        pyarrow.field(
            "test_kullback_leibler_divergence", pyarrow.list_(pyarrow.float32())
        ),
        pyarrow.field(
            "train_kullback_leibler_divergence", pyarrow.list_(pyarrow.float32())
        ),
    ]
    ...
    column_name_mapping = {
        ...
        "test_mean_squared_logarithmic_error": "test_mean_squared_logarithmic_error",
        "train_mean_squared_logarithmic_error": "train_mean_squared_logarithmic_error",
        "train_hinge": "train_hinge",
        "train_squared_hinge": "train_squared_hinge",
        "train_cosine_similarity": "train_cosine_similarity",
        "train_kullback_leibler_divergence": "train_kullback_leibler_divergence",
    ...
    with CursorManager(credentials) as cursor:
        ...
        q += sql.SQL(
            " RIGHT JOIN experiment_ e ON (e.experiment_id = r.experiment_id) "
        )
        q += sql.SQL(" ").join(
            [
                sql.SQL(
                    " left join parameter_ {} on ({}.kind = {} and r.run_parameters @> array[{}.id]) "
                ).format(
                    sql.Identifier(p),
                    sql.Identifier(p),
                    sql.Literal(p),
    ...
    def download_chunk(chunk):
        while True:
            try:
                ...
                q += sql.SQL(" FROM ( ")
                q += sql.SQL(
                    " SELECT r.*, e.num_free_parameters num_free_parameters, e.widths widths, e.network_structure network_structure, EXTRACT(epoch FROM s.start_time) start_time, EXTRACT(epoch FROM s.update_time) update_time, d.command command, "
                )
                ...
                q += sql.SQL(
                    " FROM run_ r JOIN experiment_ e ON (r.experiment_id = e.experiment_id) "
                )
