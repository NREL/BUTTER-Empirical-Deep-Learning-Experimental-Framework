...
;"""
            ...
            values_sql = ",".join(
                (
                    cursor.mogrify("(" + (",".join(["%s" for _ in e])) + ")", e).decode(
                        "utf-8"
                    )
                    for e in values
                )
    ...
    def convert_row(self, row):
        ...
        run_parameter_ids = self._parameter_map.to_sorted_parameter_ids(
            run_parameters, self.cursor
        )

        experiment_parameter_ids = self._parameter_map.to_sorted_parameter_ids(
            experiment_parameters, self.cursor
        )
