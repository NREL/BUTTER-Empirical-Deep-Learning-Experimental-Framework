...
def do_work(args):
    ...
    column_selection = SQL(", ").join(
        [SQL("e.{col} {col}").format(col=Identifier(c)) for c in experiment_columns]
        + [
        ...
        for row in source_records:
            ...
            try:
                result_records.append(
                    convert_run(old_parameter_map, result_logger, row, connection)
                )
...
def convert_run(
    old_parameter_map, result_logger, row, connection
        ...
        experiment = TrainingExperiment(
            ...
            model=DenseBySize(
                ...
                output=Dense.make(
                    ...
                    layer_config
                    | {
                        "activation": src_parameters.get("output_activation", None),
                    },
                ),
                shape=shape,
    ...
    computed_structure {json.dumps(marshal.marshal(network.structure),indent=1)}"""
        ...
        result_record.experiment_attrs["old_experiment_id"] = get_cell("experiment_id")

        result_record.run_data.update(
            {
                "run_id": get_cell("run_id"),
                "python_version": src_parameters.get("python_version", None),
                "platform": get_cell("platform"),
                "tensorflow_version": src_parameters.get("tensorflow_version", None),
                "host_name": get_cell("hostname"),
                "slurm_job_id": get_cell("slurm_job_id"),
                "git_hash": get_cell("git_hash"),
                "task_version": int(src_parameters.get("task_version", 0)),
            }
        )
        ...
        for k in [
            ...
            if get_cell(k):
                result_record.experiment_tags[k] = True
