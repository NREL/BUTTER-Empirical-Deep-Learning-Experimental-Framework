...
class CountFreeParametersVisitor:
    ...
    def _get_count_for_conv_layer(
        ...
        input_conv_shape, input_channels = target.to_conv_shape_and_channels(
            target.input.computed_shape
        )

        # num_nodes = num_nodes_per_filter * target['filters']
        weights_per_filter = input_weights_per_node_channel * input_channels + (
            1 if target.use_bias else 0
        )
