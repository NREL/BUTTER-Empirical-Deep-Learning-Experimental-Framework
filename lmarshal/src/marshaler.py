...
class Marshaler(CommonMarshaler):
    ...
    def marshal_untyped(
        ...
        source_id = id(source)
        if (
            source_id in vertex_index
        ):  # if source is already indexed, return its reference index
    ...
    def marshal_dict(marshaler: "Marshaler", source: Mapping) -> dict:
        return Marshaler.marshal_untyped(marshaler, source, Marshaler.marshal_bare_dict)
    ...
    def marshal_typed(
        ...
        return marshaler.marshal_untyped(
            marshaler, source, type_checked_object_marshaler
        )
    ...
    def enum_marshaler(
        ...
        return {
            marshaler.marshal_key(marshaler._config.enum_value_key): marshaler.marshal(
                source.value
            )
        }
    ...
    def initialize_type_map(
        type_map: Dict[Type, RawObjectMarshaler], config: MarshalConfig
    ) -> None:
