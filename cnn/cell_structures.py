        ...
        def __init__(self, nodes, operations, channels, batch_norm, activation, 
            ...
            for i in range(nodes-1):
                for j in range(by_node[i]):
                    setattr(self, f'operation_{i}_{j}', operation_dict[operations[i][j]](channels, batch_norm, activation, 
                        kernel_regularizer, bias_regularizer, activity_regularizer, **kwargs))
    ...
    class ParallelConcatCell(layers.Layer):
        ...
        def __init__(self, nodes, operations, channels, batch_norm, activation, 
            ...
            for i in range(nodes):
                for j in range(self.by_node[i]):
                    setattr(self, f'operation_{i}_{j}', operation_dict[operations[i][j]](self.channels[i], batch_norm, activation, 
                        kernel_regularizer, bias_regularizer, activity_regularizer, **kwargs))
